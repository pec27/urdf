...
```math
f^{\rm iso}_\sigma ({\bf r} , {\bf s} ) = \sum_{n=0}^\infty \; \sum_{m=-n}^n \frac{2n+2}{\pi \epsilon_m} Z^m_n( {\bf r} ) Z^m_n({\bf s} ) e^{-2 \sigma^2 \lambda^m_n} ,
```
(where ***r*** and ***s*** are the reflected and specular directions) but for faster convergence this is re-summed as the series
